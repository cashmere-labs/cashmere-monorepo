...
export class Api {
  ...
  async pendingTransactionsList(account: string): Promise<{ count: number; items: SwapData[] }> {
    return (
      await this.client.get('/api/transactionsList', {
        params: {
          type: 'pending',
          account,
        },
  ...
  async getUndetectedTxids(txIds: string[]): Promise<string[]> {
    return (
      await this.client.get('/api/getUndetectedTxids', {
        params: {
          txIds: txIds.join(','),
        },
      })
  ...
  async getSwapParams(
    ...
    return (
      await this.client.get('/api/params', { params: { srcChainId, srcToken, amount, dstChainId, dstToken, receiver } })
    ).data;
  }
  ...
  async getSwapEstimate(
    ...
    return (await this.client.get('/api/estimate', { params: { srcChainId, srcToken, amount, dstChainId, dstToken } }))
      .data;
  }
