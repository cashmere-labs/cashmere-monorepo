...
export class AuthStore {
  ...
  updateNonceRequestId() {
    let requestId, b;
    for (requestId = '', b = 36; requestId.length < 40; ) requestId += ((Math.random() * b) | 0).toString(b);
  ...
  private updateRefreshToken(token?: string) {
    ...
    if (token) store.set('refreshToken', token);
    else store.remove('refreshToken');
  }
  ...
  @action async logout() {
    if (this.token) await this.rootStore.api.logout();
  ...
  prepareMessage(siweMessage: SiweMessage) {
    ...
    if (siweMessage.expirationTime) messageLines.push(`Expiration Time: ${siweMessage.expirationTime}`);
    if (siweMessage.notBefore) messageLines.push(`Not Before: ${siweMessage.notBefore}`);
    if (siweMessage.requestId) messageLines.push(`Request ID: ${siweMessage.requestId}`);
    if (siweMessage.resources) {
      messageLines.push('Resources:');
      siweMessage.resources.forEach((resource) => messageLines.push(`- ${resource}`));
    }
